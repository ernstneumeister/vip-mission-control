...
export default function TasksPage({ agents, searchQuery }: Props) {
  ...
  const initialTab = searchParams.get('tab') as SubTab || 'tasks';
            ...
            {[
              ...
              <button
                ...
                className={`w-[36px] h-[32px] flex items-center justify-center text-[14px] border-r last:border-r-0 border-border transition-colors ${
                  viewMode === v.mode ? 'bg-muted' : 'bg-card hover:bg-muted/50'
                }`}
              >
                {v.icon}
              </button>
    ...
    <>
      <div className="flex items-center justify-between mb-4 gap-4">
        ...
        <button
          ...
          className="h-[36px] px-4 bg-card border border-border hover:bg-muted/50 text-foreground text-[14px] font-medium rounded-lg transition-colors flex items-center gap-2"
        >
          ...
          {tasks.map((task, idx) => {
            ...
            return (
              <div
                key={task.id}
                onClick={() => navigate(`/tasks/${task.id}`)}
                className={`flex items-center gap-3 px-4 py-3 hover:bg-muted/50 cursor-pointer transition-colors ${idx < tasks.length - 1 ? 'border-b border-border/50' : ''}`}
              >
